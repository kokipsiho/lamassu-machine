...
var pDelay = require('delay')

var puloonData = require('./puloon_data')

const MockBinding = SerialPort.Binding
MockBinding.createPort('/dev/mock', { echo: false, record: false })
...
function buildResponseFrame (commandCode, errorCode, parameters) {
  var frame = [0x01, 0x30, 0x02, commandCode, errorCode]
  frame = frame.concat(parameters, 0x03)
  var bcc = computeBcc(frame)
  frame = frame.concat(bcc)
  var buffer = Buffer.from(frame)
...
PuloonRs232.prototype._process = function _process (data) {
  ...
  if ((this.state === 'idle' || this.state === 'waitAck') && data[0] === 0xff) return
...
PuloonRs232.prototype._send = async function _send (command, name, cb) {
  this.setState('waitAck')
  this.responseCallback = cb || null
  this.serial.write(command)

  // Start of mock responses
  if (command[3] === 68) {
    await pDelay(2000)
    console.log('Sending first reset response');
    this.serial.binding.emitData(Buffer.from([ACK])) // send ACK
    this.serial.binding.emitData(buildResponseFrame(0x44, 0x20, [null]))
    await pDelay(100)
    this.serial.binding.emitData(Buffer.from([EOT, 0xff])) // send EOT
    
    await pDelay(2000)
    console.log('Sending second reset response');
    this.serial.binding.emitData(Buffer.from([ACK])) // send ACK
    this.serial.binding.emitData(buildResponseFrame(0x44, 0x20, [null]))
    await pDelay(100)
    this.serial.binding.emitData(Buffer.from([EOT, 0xff])) // send EOT
  ...
  if (command[3] === 103) {
    await pDelay(1000)
    console.log('Sending get serial response');
    this.serial.binding.emitData(Buffer.from([ACK])) // send ACK
    this.serial.binding.emitData(buildResponseFrame(0x67, 0x20, ['123456']))
    await pDelay(100)
    this.serial.binding.emitData(Buffer.from([EOT, 0xff])) // send EOT
  } // end get serial
  if (command[3] === 94) {
    await pDelay(1000)
    console.log('Sending set bill length');
    this.serial.binding.emitData(Buffer.from([ACK])) // send ACK
    this.serial.binding.emitData(buildResponseFrame(0x5E, 0x20, [null]))
    await pDelay(100)
    this.serial.binding.emitData(Buffer.from([EOT, 0xff]))
  } // end bill length
}
